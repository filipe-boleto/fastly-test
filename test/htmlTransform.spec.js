...
function buildSurfaceDecisions(content, cssSelector = 'h1') {
    return {
...
async function getResultText(content, cssSelector) {
    const response = new Response(inputHtml, { headers: { 'Content-Type': 'text/html' } })
    const result = await handleSurfaceComponents(response, buildSurfaceDecisions(content, cssSelector), env)
    return result.text()
}
...
describe('MonetizationOS Proxy', () => {
    it.each([
        {
            name: 'before',
            content: { before: [{ type: 'html', content: 'BEFORE' }] },
        ...
        },
        {
            ...
            content: {
                before: [
                    { type: 'html', content: '<p>1</p>' },
                    { type: 'html', content: '<p>2</p>' },
                    { type: 'html', content: '<p>3</p>' },
                ],
            },
        ...
        {
            name: 'after',
            content: { after: [{ type: 'html', content: 'AFTER' }] },
        ...
        {
            name: 'prepend',
            content: { prepend: [{ type: 'html', content: '<p>PREPEND</p>' }] },
        ...
        {
            name: 'append',
            content: { append: [{ type: 'html', content: '<p>APPEND</p>' }] },
        ...
        },
        {
            name: 'append + after',
            content: {
                after: [{ type: 'html', content: '<p>AFTER</p>' }],
                append: [{ type: 'html', content: '<p>APPEND</p>' }],
            },
        ...
        },
        {
            ...
            content: {
                remove: true,
                append: [{ type: 'html', content: '<p>APPEND</p>' }],
                prepend: [{ type: 'html', content: '<p>PREPEND</p>' }],
            },
            expected: `<body><head>${componentsTag}</head></body>`,
        },
        {
            name: 'ignore custom',
            content: {
                before: [{ type: 'custom', content: 'UNKNOWN' }],
            },
