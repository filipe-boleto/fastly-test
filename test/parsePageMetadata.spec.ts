...
import getSurfaceDecisions from '../src/3-surface-decisions/getSurfaceDecisions'
import fetchSurfaceDecisions from '../src/3-surface-decisions/fetchSurfaceDecisions'

vi.mock('../src/3-surface-decisions/fetchSurfaceDecisions', () => ({
    default: vi.fn(),
}))
...
describe('pageMetadata in surface decisions', () => {
    const env = {
        ...
        ORIGIN_URL: '',
        INJECT_SCRIPT_URL: '',
        MONETIZATION_OS_HOST: '',
        MONETIZATION_OS_ENDPOINTS_PREFIX: '',
    ...
    it('passes parsed meta tags to fetchSurfaceDecisions', async () => {
        ...
        await getSurfaceDecisions({ request } as FetchEvent, env, response)
