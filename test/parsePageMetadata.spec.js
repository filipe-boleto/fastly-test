...
import { parsePageMetadata } from '../src/2-rewrite-origin-response/parsePageMetadata.js'
import getSurfaceDecisions from '../src/3-surface-decisions/getSurfaceDecisions.js'
import fetchSurfaceDecisions from '../src/3-surface-decisions/fetchSurfaceDecisions.js'

vi.mock('../src/3-surface-decisions/fetchSurfaceDecisions.js', () => ({
    default: vi.fn(),
}))
...
describe('pageMetadata in surface decisions', () => {
    const env = {
        SURFACE_SLUG: 'test-surface',
        ANONYMOUS_SESSION_COOKIE_NAME: 'anon-session',
        AUTHENTICATED_USER_JWT_COOKIE_NAME: 'user-jwt',
    }

    beforeEach(() => {
        vi.mocked(fetchSurfaceDecisions).mockResolvedValue(null)
    })

    it('passes parsed meta tags to fetchSurfaceDecisions', async () => {
        const html = `<html><head>
            <meta name="category" content="subscriber-only">
            <meta property="article:published_time" content="2026-03-09T15:13:00-0500">
            <meta property="og:title" content="Breaking News">
        ...
        const request = new Request('https://test.example/index.html')
        const response = new Response(html, { headers: { 'Content-Type': 'text/html' } })

        await getSurfaceDecisions(request, env, response)

        expect(vi.mocked(fetchSurfaceDecisions)).toHaveBeenCalledWith(
            env,
    ...
    it('passes empty pageMetadata when origin has no meta tags', async () => {
        ...
        expect(vi.mocked(fetchSurfaceDecisions)).toHaveBeenCalledWith(
            env,
            expect.objectContaining({ pageMetadata: {} }),
        )
    })
})
